...
map_legend_data <- tibble(
  deaths_group = factor(
    c("<5K", "5–20K", "20–50K", "50–150K", "≥150K"),
    levels = names(map_palette)
  ),
  x = seq_len(5)
)

map_legend <- ggplot(map_legend_data) +
  annotate(
    "text", x = 0, y = 1, label = "Avoided deaths",
    hjust = 0, size = 9 / .pt, family = "sans"
  ) +
  geom_tile(
    aes(x = x * 1.55 + 1.40, y = 1, fill = deaths_group),
    width = 0.30,
    height = 0.34,
    colour = "grey45",
    linewidth = 0.25
  ...
  geom_text(
    aes(x = x * 1.55 + 1.62, y = 1, label = deaths_group),
    hjust = 0,
    size = 9 / .pt,
    family = "sans"
  ) +
  scale_fill_manual(values = map_palette, guide = "none") +
  coord_cartesian(xlim = c(0, 10.0), ylim = c(0.72, 1.28), clip = "off") +
  theme_void(base_family = "sans") +
  theme(
...
panel_a_map <- panel_a +
  ...
  theme(
    ...
    plot.tag.position = c(0.21, 0.99)
  )

panel_a_large <- plot_spacer() +
  inset_element(
    panel_a_map,
    left = -0.125,
    bottom = 0.04,
...
panel_a_with_income <- (panel_a_large | panel_income) +
  plot_layout(widths = c(2.05, 1))

fig5 <- panel_a_with_income / panel_b / panel_c +
  plot_layout(heights = c(1.18, 0.86, 0.90)) +
  plot_annotation()
