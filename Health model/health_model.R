...
base_dir <- '/Users/zhongcideng/Desktop/FoodBalance2013-/optimized/optmized/output/Health model'
...
step1_wb <- createWorkbook()
addWorksheet(step1_wb, "detail")
writeData(step1_wb, "detail", step1_detail_df)
addWorksheet(step1_wb, "age_summary")
writeData(step1_wb, "age_summary", step1_age_summary_df)
addWorksheet(step1_wb, "category_map_Ori")
writeData(step1_wb, "category_map_Ori", category_map_ori_df)
addWorksheet(step1_wb, "category_map_Opt")
writeData(step1_wb, "category_map_Opt", category_map_opt_df)
saveWorkbook(step1_wb, step1_output_path, overwrite = TRUE)

#----step two-----
step1_detail_df <- read_excel('/Users/zhongcideng/Desktop/FoodBalance2013-/optimized/optmized/output/Health model/health_food_intake_step1.xlsx')
# PIF calculation
...
filled_groups <- character(nrow(raw_risk_df))
for (i in seq_len(nrow(raw_risk_df))) {
  if (!is.na(raw_risk_df$`Food group`[[i]])) {
    current_group <- raw_risk_df$`Food group`[[i]]
  }
  filled_groups[[i]] <- current_group
}
raw_risk_df$`Food group` <- filled_groups
...
step2_wb <- createWorkbook()
addWorksheet(step2_wb, "risk_factors")
writeData(step2_wb, "risk_factors", risk_df)
addWorksheet(step2_wb, "pif_detail")
writeData(step2_wb, "pif_detail", step2_pif_detail_df)
saveWorkbook(step2_wb, step2_output_path, overwrite = TRUE)
...
step3_wb <- createWorkbook()
addWorksheet(step3_wb, "delta_deaths_detail")
writeData(step3_wb, "delta_deaths_detail", step3_detail_df)
addWorksheet(step3_wb, "delta_deaths_summary")
writeData(step3_wb, "delta_deaths_summary", step3_age_summary_df)
saveWorkbook(step3_wb, step3_output_path, overwrite = TRUE)
